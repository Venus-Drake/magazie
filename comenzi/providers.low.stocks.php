...
<?php
    ...
    if(isset($_GET['alarm']) && $_GET['alarm'] != '') 
	{
		...
		{
		    $que = "SELECT * FROM `furnizori_materiale` GROUP BY `furnizor` ORDER BY `furnizor`";
		    if($run = mysql_query($que))
		    {
     		...
     		<DIV ID = "scrollableDIV" STYLE = "WIDTH: 99.5%; MAX-HEIGHT: 40vw; OVERFLOW: AUTO; PADDING: 0; BORDER: 2px SOLID BLACK;">
			    ...
			    {
				    $furnizorADD = "SELECT `denumire`, `furnizor`, `cod_SAP` FROM `magazie_stoc` ORDER BY `furnizor`";
				    if($furnizorRUN = mysql_query($furnizorADD))
				    {
					    if(mysql_num_rows($furnizorRUN) > 0)
					    {
						    while($furnizorROW = mysql_fetch_assoc($furnizorRUN))
						    {
							    $denumire = $furnizorROW['denumire'];
							    $furnizor = $furnizorROW['furnizor'];
							    $codSAP = $furnizorROW['cod_SAP'];
							    $adding = "INSERT INTO `furnizori_materiale` VALUES('','$furnizor','','','','','$denumire','$codSAP')";
							    if($addingRUN = mysql_query($adding))
							    {
                ...
                if(mysql_num_rows($run) > 0)
                {
				    while($row = mysql_fetch_assoc($run))
				    {
						...
						else 
						{
							...
							if($count == mysql_num_rows($run) && $codSAP != '' && $cantOPT >= $cantitate) echo '<TR><TD STYLE = "BORDER: 1px SOLID BLACK; FONT-SIZE: 1.2vw; TEXT-ALIGN: CENTER;">'.$codSAP.'</TD>
							  <TD STYLE = "BORDER: 1px SOLID BLACK; FONT-SIZE: 1.2vw; TEXT-ALIGN: CENTER;">'.$produs.'</TD>
							  <TD STYLE = "BORDER: 1px SOLID BLACK; FONT-SIZE: 1.2vw; TEXT-ALIGN: CENTER;">'.$cantitate.'</TD>
							  <TD STYLE = "BORDER: 1px SOLID BLACK; FONT-SIZE: 1.2vw; TEXT-ALIGN: CENTER;">'.$UM.'</TD>
							  <TD STYLE = "BORDER: 1px SOLID BLACK; FONT-SIZE: 1.2vw; TEXT-ALIGN: CENTER;">'.$detalii.'</TD>
