	...
	<DIV ID = "showSTOCKoptions" STYLE = "DISPLAY: NONE; BORDER: 2px SOLID BLACK; BACKGROUND-COLOR: WHITE; BORDER-RADIUS: 20px; MARGIN-LEFT: 1vw; WIDTH: 18vw; MARGIN-TOP: 0.5vw;"><BR>
	    <TABLE STYLE = "MARGIN-LEFT: 1vw;">
	        ...
	        <TD>
				...
				if(!$qmag = $connect -> query("SELECT `magazie` FROM `magazie_stoc` GROUP BY `magazie`"))
				{
				...
				if(mysqli_num_rows($qmag) > 0)
				{
					while($qmagROW = $qmag -> fetch_assoc())
					{
				...
				if(!$qGRUP = $connect -> query("SELECT `grupa_MAT` FROM `magazie_stoc` GROUP BY `grupa_MAT`"))
				{
					$mailerror = '<font STYLE = "FONT-SIZE: 1.5vw;"><center><b>FATAL ERROR!<BR>Something unexpected went wrong!<BR>MySQL Error:<BR>'.__LINE__.". ".__FILE__.":<br>".mysqli_error($connect).'<br>Please, contact program administrator at<br><a href = "mailto: warehouse-soft@ramira.ro?subject=Fatal error feedback&body=The program has returned the next fatal error: '.__LINE__.'. '.__FILE__.': Something unexpected went wrong! '.mysqli_error($connect).'">warehouse-soft@ramira.ro</a>';
					require $_SERVER['DOCUMENT_ROOT'].'/ramira/magazie/error.handler.php';
					mysqli_close($connect);	
				}
				if(mysqli_num_rows($qGRUP) > 0)
				{
					while($qGRUProw = $qGRUP -> fetch_assoc())
					{
