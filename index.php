...
<HEADER>
    ...
    <?php
		...
		$ipchk = "UPDATE `utilizatori` SET `IP.connect` = '' WHERE `IP.connect` = '$iplog'";
	 	if($iprun = mysql_query($ipchk)){}
	 	else
	 	{
    ...
    <BODY onbeforeunload="return pageReload()">
        <?php 
		    ...
		    //else echo $_SERVER['HTTP_USER_AGENT'].'. Found string to: '.strpos(strtolower($_SERVER['HTTP_USER_AGENT']),'chrome');
        ?>
